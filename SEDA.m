...
function [measures,time_so_far2] = SEDA(Data, expt)
% Usage: measures = SEDA(BatchSize, expt, Data)
%
...
% fprintf(' Running Supervised Evolving Domain Adaptation (SEDA) ');
% t_osvd = tic;
% wfig = waitbar(0, 'Supervised EDA');
...
for t=1:maxT
   ...
   % m = length(batchIndex); % The number of test data
    n = nm-length(batchIndex);               % The number of training data
    % Compute the weight of the instances       
    E = diag(sparse([ones(n,1);zeros(length(batchIndex),1)]));
    y = E*YYtotal(1:nm,:);
    ...
    lambda = zeros(nm,1);
    lambda(1:n) = fliplr(weight(1:n)')';
    
nEmpty = length(find(weight(1:n)==0));
uu2(1:nEmpty,:)=[];
y(1:nEmpty,:)=[];
lambda(1:nEmpty)=[];

    lambda = uu2'*sparse(diag(lambda));
    ...
    temp_Yt = [temp_Yt ;Yt(batchIndex,1)];
    [v1, C, Ps, mean_x_old, mean_y_old, N] = IPLS(Xt(batchIndex,:)',...
        Yt(batchIndex,1)',dim, mean_x_old, mean_y_old, v1, C, N);
...
%     waitbar(t*BatchSize/T, wfig, sprintf('Supervised EDA %2.1f/%2.1f (s)', time_so_far, expected_time));
% 
